        ...
        return async (request, response, next) => {
            try {
                ...
                if (!controllers[controllerName] || controllers[controllerName] === undefined) {
                    this.handleError(`request sent to controller "${controllerName}" which has not been defined`,
                ...
                new Logger().logger.error(`Server on port ${config.URL_PORT} shut down`);
